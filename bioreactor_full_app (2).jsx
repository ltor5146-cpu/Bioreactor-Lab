...
function LandingForm({ onAccessGranted }) {
  ...
  const handleSubmit = async () => {
    ...
    // Send to Firebase - Single Student Document (Persistent Profile + Current Session)
    const sessionData = {
    ...
    if (db) {
      setDoc(doc(db, "students", form.studentId.trim()), sessionData, { merge: true })
        ...
        <div style={formPanel}>
          ...
          <div style={{ marginBottom: 26 }}>
            ...
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 20, marginBottom: 10 }}>
              <img src="/logo1.jpg" alt="Logo 1" style={{ height: 60, width: "auto", objectFit: "contain" }} />
              ...
              <div style={{ display: "flex", gap: 8 }}>
                <img src="/logo2.jpg" alt="Logo 2" style={{ height: 50, width: "auto", objectFit: "contain" }} />
                <img src="/logo3.jpg" alt="Logo 3" style={{ height: 50, width: "auto", objectFit: "contain" }} />
              </div>
            </div>
...
// ─── MAIN COMPONENT ───────────────────────────────────────────────────────────
function BioreactorSim({ studentId, sessionKey, studentName, entryTime }) {
  ...
  // 1. Live Heartbeat (every 30s)
  useEffect(() => {
    if (phase !== "playing" || !sessionKey) return;
    const hb = setInterval(() => {
      const g = gameRef.current;
      const duration = ((new Date() - new Date(entryTime)) / 60000).toFixed(2);
      const data = {
        "Exit Time": new Date().toISOString(),
        "Current Session Duration (min)": duration,
        "Current Stats": {
          "Current Culture Health": g.health.toFixed(1) + "%",
          "Current Culture Density": g.density.toFixed(1) + "%",
          "Growth Phase": growthPhase,
          "Live Temperature": g.params.temperature.toFixed(1) + "°C",
          "Live pH": g.params.ph.toFixed(2),
          "Live O2": g.params.oxygen.toFixed(0) + "%",
          "Running Score": g.score
        }
      };
      updateDoc(doc(db, "students", studentId), data).catch((e) => console.error("Firebase Error:", e));
    }, 30000);
    return () => clearInterval(hb);
  }, [phase, sessionKey, entryTime, growthPhase]);

  // 2. Cumulative Trials History
  useEffect(() => {
    if ((phase === "won" || phase === "dead") && studentId) {
      const trial = {
        trialId: round,
        organism: species,
      ...
      const finalUpdate = {
        "Latest Trial": trial,
        "Trials History": arrayUnion(trial)
      };
      updateDoc(doc(db, "students", studentId), finalUpdate).catch((e) => console.error("Firebase Error:", e));
    }
  }, [phase, studentId, sessionKey, species, round, score, timeLeft, density, health]);
...
// ══════════════════════════════════════════════════════════════════════════════
function BioreactorSimulator({ studentName, studentId, sessionKey, entryTime }) {
  return <BioreactorSim studentName={studentName} studentId={studentId} sessionKey={sessionKey} entryTime={entryTime} />;
}
